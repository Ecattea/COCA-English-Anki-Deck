...
## 当前版本概览（2026-04-04）
- **严格发布版规模：** 21,788 张原子卡
- **lemma 覆盖：** 严格发布版覆盖 4,006 个 lemma
- **例句覆盖：** 每张卡最多 5 条例句
- **元数据门槛：** 正式发布卡中，缺 IPA、缺单词音频、缺释义、缺例句的卡片均为 0
- **牌组结构：** 按 COCA 词频划分为 5 个约 1000 rank 的子牌组

## 本次完善的重点
- **原子化更干净：** 使用更严格的 sense-level parser，尽量保证一张卡只对应一个明确释义或一个明确用法。
- **JSON 边界处理更完整：** 补充了更多 Merriam-Webster 结构的解析，例如嵌套义项、usage 容器以及额外的例句字段。
- **正式版质量更高：** 严格发布版已经排除了表格不安全单元格、无例句卡和缺音频卡。
- **模板同步升级：** 正反面模板现在都一致支持最多 5 条例句。
- **重复卡更少：** `(word, part of speech, definition)` 重复卡从旧项目的 1,462 行降到了本次版本的 2 行。
...
## 仓库内容说明
- `COCA-English/notes.csv`：用于构建卡组的导入 CSV
- `COCA-English/templates/`：笔记模板和样式文件
- `COCA-English/medias/`：共享媒体资源
- 当前版本的发布说明和下载内容请查看 [Releases 页面](https://github.com/Ecattea/COCA-English-Anki-Deck/releases)
